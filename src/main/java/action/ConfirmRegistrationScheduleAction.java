...
public class ConfirmRegistrationScheduleAction implements IAction {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws NSCOException {
		...
		try {
    		...
    		if(result == 1) {
				session.setAttribute("registrationMessage", "スケジュールを登録しました");
    		}else {
    			session.setAttribute("registrationMessage", "スケジュールを登録できませんでした");
    		}
			
    		session.setAttribute("registrationSchedule", null);
